      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template #default="scope">
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)"
                     v-hasPermi="['merchant:dish:edit']">修改
          </el-button>
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)"
                     v-hasPermi="['merchant:dish:remove']">删除
          </el-button>
        </template>
      </el-table-column>
    </el-table>
        ...
        <el-table :data="dishFlavorList" :row-class-name="rowDishFlavorIndex"
          ...
          <el-table-column label="口味名称" prop="name" width="150">
            <template #default="scope">
              ...
              <el-select v-model="scope.row.name" placeholder="请选择口味名称" @change="changeFlavorName(scope.row)">
                <el-option
                    v-for="dictFlavor in dishFlavorListSelect"
                    :key="dictFlavor.name"
                    :label="dictFlavor.name"
                    :value="dictFlavor.name"
                ></el-option>
              </el-select>
            </template>
          </el-table-column>
          <el-table-column label="口味列表" prop="value" width="350">
            <template #default="scope">
              ...
              <el-select v-model="scope.row.value"   style="width: 90%"
                         @focus="focusFlavorName(scope.row)" placeholder="请选择口味列表" multiple>
                <el-option
...
//----------------------------
// 定义口味名称和口味列表的静态数据
const dishFlavorListSelect = reactive([
  {name: "辣度", value: ["不要葱", "不要蒜", "不要香菜", "不要辣"]},
  {name: "忌口", value: ["不辣", "微辣", "中辣", "重辣"]},
...
function changeFlavorName(row) {
  ...
  row.value = [];
  // 遍历数据，找到匹配的口味名称
  checkValueList.value = dishFlavorListSelect.find(item => item.name === row.name).value;
}
