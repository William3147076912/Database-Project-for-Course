<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryRef" :inline="true" v-show="showSearch" label-width="100px">
      <!--      <el-form-item label="选修的课程" prop="name">
              <el-input
    ...
    <el-table v-loading="loading" :data="enrollmentList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="选修的课程" align="center" prop="name"/>
      <el-table-column label="选课时间" align="center" prop="enrollmentTime" width="180">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template #default="scope">
          ...
          <el-button link type="warning" icon="close" @click="unselectCourse(scope.row)"
                     v-hasPermi="['enrollment:enrollment:remove']">退选
          </el-button>
        </template>
      </el-table-column>
    </el-table>
...
/** 退选按钮操作 */
function unselectCourse(row) {
  const _enrollmentIds = row.enrollmentId || ids.value;
  proxy.$modal.confirm('是否确认退选选课记录编号为"' + _enrollmentIds + '"的数据项？').then(function () {
